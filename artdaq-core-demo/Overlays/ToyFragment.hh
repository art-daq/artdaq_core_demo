...
public:
   ...
   */
	...
	static_assert(sizeof(Metadata) == Metadata::size_words * sizeof(Metadata::data_t),
	              "ToyFragment::Metadata size changed");
   ...
   */
	explicit ToyFragment(artdaq::Fragment const& f)
	    : artdaq_Fragment_(f) {}
   ...
   */
	...
	{
		return std::find_if(dataBeginADCs(), dataEndADCs(), [&](adc_t const adc) -> bool { return (adc >> daq_adc_bits); });
	}
   ...
   */
	bool fastVerify(int daq_adc_bits) const { return (findBadADC(daq_adc_bits) == dataEndADCs()); };
   ...
   */
	static size_t adc_range(int daq_adc_bits) { return (1ul << daq_adc_bits) - 1; }
