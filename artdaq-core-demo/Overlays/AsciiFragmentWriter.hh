...
inline demo::AsciiFragmentWriter::AsciiFragmentWriter(artdaq::Fragment& f) : AsciiFragment(f), artdaq_Fragment_(f) {
  if (!f.hasMetadata() || f.dataSizeBytes() > 0) {
    throw cet::exception(
        "Error in AsciiFragmentWriter: Raw artdaq::Fragment object does not appear to consist of (and only of) its own "
        "header + the AsciiFragment::Metadata object");
  }
...
inline size_t demo::AsciiFragmentWriter::chars_to_words_(size_t nChars) {
  ...
  return (mod == 0) ? nChars / chars_per_word_() : nChars / chars_per_word_() + 1;
}
